...
## 日常使用与常见命令
**我们支持用户为不同的读写权限，任务复杂度分别配置不同供应厂商的模型。简单的对话问答和任务这里推荐open router的openrouter/free模型或者glm的4.7 flash。后者会有更多的上游繁忙问题。前者响应速度很快，但是每次路由到什么不知名模型你别管——如果执行复杂任务，一个任务会出现路由到多个不同的free模型，并且有概率存在上游繁忙导致任务失败的情况，谨慎考虑。**
