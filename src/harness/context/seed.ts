...
import type { InspectedPR } from '../../github/pull-request.ts';
import type { Trace } from '../trace.ts';
import { excerpt } from './policy.ts';
import type { CurrentBaseSnapshot } from '../runtime.ts';

export async function seedContext(pr: InspectedPR, ws: WorkspaceState, trace: Trace, currentBase?: CurrentBaseSnapshot) {
  const currentBaseTipSha = currentBase?.sha ?? ws.mainSha;
  const currentBaseRef = currentBase?.ref ?? pr.pr.base.ref;
  ...
  return { repository: pr.repository.full_name, pr_number: pr.pr.number, title: pr.pr.title,
    body: excerpt(pr.pr.body ?? ''), base_sha: pr.snapshot.base.sha, head_sha: headSha,
