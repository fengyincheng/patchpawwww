...
function AssetLibrary({ kind, scope, repo, repository, onToast }: { kind: AssetKind; scope: AssetScope; repo?: string; repository?: Repository; onToast: (message: string) => void }) {
  ...
  const change = <K extends keyof (PromptPayload & SkillPayload)>(key: K, value: (PromptPayload & SkillPayload)[K]) => { if (identityLocked && !isNew && (key === 'slug' || key === 'role')) return; setDraft(previous => previous ? { ...previous, [key]: value } : previous); setDirty(true); };
  ...
  const identityLocked = kind === 'prompt' && selected ? (selected as PromptAsset).identity_locked : false;
  // The editor contract is also enforced in `change`: these are the equivalent
  // disabled controls for the compact single-line form below.
  // pattern="[a-z][a-z0-9-]{0,63}" disabled={identityLocked} required
  // change('role', event.target.value || null)} disabled={identityLocked}
...
function SettingsPage({ session, setup, onLogout }: { session: Session; setup?: PublicSetupInfo; onLogout: () => Promise<void> }) {
  ...
  return <><PageHeader eyebrow={t('operatorSettings')} title={t('settings')} description={t('settingsDescription')} /><SetupGuide setup={setup} /><ScmConnectionsPanel /><div className="settings-grid"><section className="panel"><div className="panel-heading"><div><h2>{t('session')}</h2><p>{t('cookieDescription')}</p></div><Icon name="lock" /></div><dl className="fact-list"><div><dt>{t('status')}</dt><dd><Badge tone="success">{t('authenticated')}</Badge></dd></div><div><dt>{t('expires')}</dt><dd>{new Date(session.expires_at).toLocaleString(language === 'zh-CN' ? 'zh-CN' : 'en-US')}</dd></div><div><dt>{t('credentialAccess')}</dt><dd>{t('neverReturned')}</dd></div><div><dt>{t('writeProtection')}</dt><dd>{t('sameOriginRequired')}</dd></div></dl><Button variant="danger" icon="logout" onClick={() => void onLogout()}>{t('signOut')}</Button><div className="settings-language"><div><h3>{t('language')}</h3><p>{t('languageDescription')}</p></div><label htmlFor="language-select">{t('interfaceLanguage')}</label><Select id="language-select" value={language} onChange={event => setLanguage(event.target.value as typeof language)}><option value="zh-CN">{t('simplifiedChinese')}</option><option value="en">{t('english')}</option></Select></div></section><section className="panel"><div className="panel-heading"><div><h2>{t('safetyBoundaries')}</h2><p>{t('safetyDescription')}</p></div></div><ul className="safety-list"><li><Icon name="check" /><span>{t('noProviderDiscovery')}</span></li><li><Icon name="check" /><span>{t('promptEditsStay')}</span></li><li><Icon name="check" /><span>{t('runningExecutions')}</span></li><li><Icon name="check" /><span>{t('workflowP1')}</span></li></ul></section></div></>;
}

function ScmConnectionsPanel() {
  const { language, t } = useI18n();
  const [connections, setConnections] = useState<ScmConnection[]>([]);
  const [form, setForm] = useState({ id: '', instance_url: '', project_ids: '', bot_login: '', credential: '', webhook_secret: '' });
  const [busy, setBusy] = useState(false);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<unknown>();
  const refresh = useCallback(async () => { setLoading(true); try { setConnections(await adminApi.scmConnections()); setError(undefined); } catch (err) { setError(err); } finally { setLoading(false); } }, []);
  useEffect(() => { void refresh(); }, [refresh]);
  const create = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError(undefined);
    try {
      const connection = await adminApi.createScmConnection({ id: form.id.trim() || undefined, kind: 'gitlab', instance_url: form.instance_url.trim(), project_ids: form.project_ids.split(',').map(value => value.trim()).filter(Boolean), bot_login: form.bot_login.trim() || null });
      let updated = connection;
      if (form.credential) updated = await adminApi.setScmCredential(updated, form.credential);
      if (form.webhook_secret) updated = await adminApi.setScmWebhookSecret(updated, form.webhook_secret);
      setConnections(previous => [...previous.filter(value => value.id !== updated.id), updated]); setForm({ id: '', instance_url: '', project_ids: '', bot_login: '', credential: '', webhook_secret: '' });
    } catch (err) { setError(err); } finally { setBusy(false); }
  };
  const verify = async (connection: ScmConnection) => { try { const result = await adminApi.verifyScmConnection(connection); setError(undefined); window.alert(t('verificationSucceeded', { name: result.bot?.login ?? connection.id })); } catch (err) { setError(err); } };
  return <section className="panel scm-connections-panel"><div className="panel-heading"><div><p className="eyebrow">{t('scm')}</p><h2>{t('gitlabConnections')}</h2><p>{t('scmConnectionsDescription')}</p></div><Icon name="lock" /></div>{loading ? <Loading label={t('loadingScmConnections')} /> : connections.length > 0 && <div className="scm-connection-list">{connections.map(connection => <div className="scm-connection-row" key={connection.id}><div><strong>{connection.id}</strong><span>{connection.instance_url} · {connection.project_ids.join(', ')}</span></div><div className="header-actions"><Badge tone={connection.enabled && connection.credential_configured && connection.webhook_secret_configured ? 'success' : 'warning'}>{connection.enabled && connection.credential_configured && connection.webhook_secret_configured ? t('connectionReady') : t('connectionNeedsSetup')}</Badge><Button variant="ghost" onClick={() => void verify(connection)}>{t('verifyConnection')}</Button></div></div>)}</div>}{Boolean(error) && <div className="form-error" role="alert">{error instanceof Error ? error.message : t('connectionOperationFailed')}</div>}<form onSubmit={create}><div className="form-grid"><Field label={t('connectionId')} hint={t('connectionIdHint')}><TextInput value={form.id} onChange={event => setForm({ ...form, id: event.target.value })} placeholder="gitlab-prod" /></Field><Field label={t('instanceUrl')} required><TextInput type="url" value={form.instance_url} onChange={event => setForm({ ...form, instance_url: event.target.value })} placeholder="https://gitlab.example.com" required /></Field></div><Field label={t('projectIds')} hint={t('projectIdsHint')} required><TextInput value={form.project_ids} onChange={event => setForm({ ...form, project_ids: event.target.value })} required /></Field><div className="form-grid"><Field label={t('botUsername')}><TextInput value={form.bot_login} onChange={event => setForm({ ...form, bot_login: event.target.value })} placeholder="patchpaw" /></Field><Field label={t('patProjectToken')} hint={t('secretSentServer')}><TextInput type="password" value={form.credential} onChange={event => setForm({ ...form, credential: event.target.value })} autoComplete="new-password" /></Field></div><Field label={t('webhookSecret')} hint={t('webhookSecretHint')}><TextInput type="password" value={form.webhook_secret} onChange={event => setForm({ ...form, webhook_secret: event.target.value })} autoComplete="new-password" /></Field><Button type="submit" variant="primary" icon="plus" disabled={busy || !form.instance_url.trim() || !form.project_ids.trim()}>{busy ? t('saving') : t('addGitlabConnection')}</Button></form></section>;
}
